...
export class CreateIssuesUseCase extends UseCase<CreateIssueCommand, Issue[]> {
    ...
    execute(): Issue[] | Promise<Issue[]> {
        ...
        const problemDictionnary: {
            [Id: string]: { video: string; category: string; issues: Issue[] };
        } = {};

        this.command.items.forEach((item) => {
            if (!problemDictionnary[`${item.video}-${item.category}`]) {
                problemDictionnary[`${item.video}-${item.category}`] = {
                    video: item.video,
                    category: item.category,
                    issues: [],
                };
            }
            problemDictionnary[`${item.video}-${item.category}`].issues.push(
                new Issue({
        ...
        for (const id in problemDictionnary) {
            ...
            const savedProblem = this.problemRepo.saveOne(
                new Problem({
                    video: item.video,
                    category: item.category,
            ...
            issuesToSave = [
                ...issuesToSave,
                ...problemDictionnary[id].issues.map((issue) => {
